...
window.unclaimGift = async function(giftName, giftId) {
    const confirmed = await showCustomModal({ type: 'confirm', title: 'Desmarcar Presente', message: `Tem certeza que deseja liberar este presente?<br><strong style="color:var(--olive-primary)">${giftName}</strong><br><br>Outra pessoa poderá marcá-lo.` });
    if (!confirmed) return;
